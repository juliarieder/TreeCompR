...
```
Airborne Laser Scanning is a powerful tool for forest management and research, offering accurate large-scale data. To quantify competition on the invidual tree level, first the ALS point cloud needs to be pre-processed. For using the data in TreeCompR, we need to segment the trees and derive an inventory table. We used publicly available laser scanning datasets from the [Bavarian Agency for Digitisation, High-Speed Internet and Surveying](https://geodaten.bayern.de/opengeodata/OpenDataDetail.html?pn=laserdaten). The package [`lidR`](https://CRAN.R-project.org/package=lidR) is a great option to load,inspect and process your ALS data and to segment individual trees. Check out the [lidRbook](https://r-lidar.github.io/lidRbook/) for their very nice detailed workflows and examples. 
...
```
```{r plot-chm, fig.height=7, fig.width=7}
...
```{r plot-CI, fig.height= 7, fig.width=8}
...
trees are being segmented based on typical allometric relations that can be defined beforehand.
...
# segment the trees (ajust epsg according to your coordinate reference system)
...
compete_inv(inv_source = inv_trees, target_source = "buff_edge", radius = 10, method = "all")
```
