...
This combination of pre-processingt and built-in data aggregation should be sufficient for a majority of datasets. If the raw data are too large to be read into memory even after clipping to the surroundings of the target tree, it makes sense to further reduce their size by voxelizing/rounding _before_ loading, e.g. in CloudCompare. Here, ideally round to 1-2 decimal places more than desired `acc_digits`, because hat the storage format may result in slight numeric differences (especially for .las/.laz) that can affect coordinate matching.
