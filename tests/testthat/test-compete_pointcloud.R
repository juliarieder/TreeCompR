...
test_that("compete_pc works for .txt file point clouds", {
  ...
  expect_no_error(
     test1 <- compete_pc(forest_source = test_path("testdata", "neighborhood.txt"),
  ...
  expect_no_error(
      test2 <- compete_pc(forest_source = test_path("testdata", "neighborhood.txt"),
                 tree_source = test_path("testdata", "tree.txt"),
  ...
  expect_no_error(
      test3 <- compete_pc(forest_source = test_path("testdata", "neighborhood.txt"),
                 tree_source = test_path("testdata", "tree.txt"),
  ...
  # test for dimensions
  expect_equal(c(length(test1), length(test2), length(test3)), c(5,5,7))
